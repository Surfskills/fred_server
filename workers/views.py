...
#         # Prepare the response with detailed offer information
#         result = []
...
#                     service = Service.objects.get(id=offer.offer_id)
#                     offer_data['details'] = ServiceSerializer(service).data
#                 elif offer.offer_type == 'software':
#                     software = SoftwareRequest.objects.get(id=offer.offer_id)
#                     offer_data['details'] = SoftwareRequestSerializer(software).data
#                 elif offer.offer_type == 'research':
#                     research = ResearchRequest.objects.get(id=offer.offer_id)
#                     offer_data['details'] = ResearchRequestSerializer(research).data
#             except (Service.DoesNotExist, SoftwareRequest.DoesNotExist, ResearchRequest.DoesNotExist):
#                 # If the underlying offer was deleted, just continue
#                 offer_data['details'] = {'error': 'Original offer no longer exists'}
...
#             'accepted_at': accepted_offer.accepted_at,
#             'completed_at': accepted_offer.completed_at,
#             'returned_at': accepted_offer.returned_at,
#             'details': None
#         }
        
#         try:
#             if accepted_offer.offer_type == 'service':
#                 service = Service.objects.get(id=accepted_offer.offer_id)
#                 offer_data['details'] = ServiceSerializer(service).data
#             elif accepted_offer.offer_type == 'software':
#                 software = SoftwareRequest.objects.get(id=accepted_offer.offer_id)
#                 offer_data['details'] = SoftwareRequestSerializer(software).data
#             elif accepted_offer.offer_type == 'research':
#                 research = ResearchRequest.objects.get(id=accepted_offer.offer_id)
#                 offer_data['details'] = ResearchRequestSerializer(research).data
#         except (Service.DoesNotExist, SoftwareRequest.DoesNotExist, ResearchRequest.DoesNotExist):
#             offer_data['details'] = {'error': 'Original offer no longer exists'}
    ...
    def post(self, request):
        ...
        for model in [Service, SoftwareRequest, ResearchRequest]:
            try:
                ...
                offer_type = model.__name__.lower()  # Get the model name dynamically (service, software_request, or research_request)
